 ...
 * Primary streaming source (sub + dub)
 */
...
export class AnimeKaiSource extends BaseAnimeSource {
        ...
        try {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                ...
                if (hasBrokenUrl && attempt < 2) {
                    logger.warn(`AnimeKai: Got broken CDN on attempt ${attempt + 1}, retrying...`, undefined, this.name);
    ...
    async getTrending(page: number = 1): Promise<AnimeBase[]> {
        try {
            const result = await this.search('', page);
