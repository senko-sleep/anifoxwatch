...
export class GogoanimeBySource extends BaseAnimeSource {
    name = 'GogoanimeBy';
    ...
    async getAnime(id: string, options?: SourceRequestOptions): Promise<AnimeBase | null> {
        try {
            const animeId = id.replace('gogoanimeby-', '');
            ...
            const type = 'TV'; // Assuming TV, can be parsed from page
            const status = 'Ongoing'; // Can be parsed
            const genres: string[] = [];
            $('.genres a').each((i, el) => {
    ...
    async getEpisodes(animeId: string, options?: SourceRequestOptions): Promise<Episode[]> {
        try {
            const id = animeId.replace('gogoanimeby-', '');
            ...
            $('.episode a').each((i, el) => {
                ...
                episodes.push({
                    id: `gogoanimeby-${id}-episode-${epNum}`,
                    number: epNum,
                    title,
                    isFiller: false,
                    hasSub: true,
            ...
            return episodes.reverse(); // Episodes are usually in reverse order
        } catch (error) {
    ...
    async getEpisodeServers(episodeId: string, options?: SourceRequestOptions): Promise<EpisodeServer[]> {
        return [{ name: 'GogoanimeBy', url: '', type: 'sub' }];
    }

    async getStreamingLinks(episodeId: string, server?: string, category: 'sub' | 'dub' = 'sub', options?: SourceRequestOptions): Promise<StreamingData> {
        try {
            const response = await axios.get(`${this.baseUrl}/${episodeId.replace('gogoanimeby-', '')}`, {
                signal: options?.signal,
            ...
            if (iframeSrc) {
                try {
                    const embedResp = await axios.get(iframeSrc, {
                        signal: options?.signal,
                        timeout: 8000,
