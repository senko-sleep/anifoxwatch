...
export class AnimeOwlSource extends BaseAnimeSource {
    name = 'AnimeOwl';
    ...
    async getEpisodes(animeId: string, options?: SourceRequestOptions): Promise<Episode[]> {
        try {
            ...
            $('.episode-item, .ep-card a').each((i, el) => {
    ...
    async getTrending(page: number = 1, options?: SourceRequestOptions): Promise<AnimeBase[]> {
        try {
            ...
            $('.anime-card').each((i, el) => {
                ...
                if (id && title) {
                    results.push({ id: `animeowl-${id}`, title, image, cover: image, description: '', type: 'TV', status: 'Ongoing', episodes: 0, episodesAired: 0, year: 0, subCount: 0, dubCount: 0, source: this.name, isMature: false, genres: [], studios: [], rating: 0 });
                }
            });
            return results;
    ...
    async getLatest(page: number = 1, options?: SourceRequestOptions): Promise<AnimeBase[]> {
        try {
            ...
            $('.anime-card').each((i, el) => {
                ...
                const image = $(el).find('img').attr('src') || '';
                if (id && title) {
                    results.push({ id: `animeowl-${id}`, title, image, cover: image, description: '', type: 'TV', status: 'Ongoing', episodes: 0, episodesAired: 0, year: new Date().getFullYear(), subCount: 0, dubCount: 0, source: this.name, isMature: false, genres: [], studios: [], rating: 0 });
                }
            });
            return results;
