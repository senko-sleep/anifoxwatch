 ...
 */
let consumetMod: any = null;
async function getMod() {
    if (!consumetMod) consumetMod = await import('@consumet/extensions');
    return consumetMod;
}
...
async function getHianimeProvider() {
    ...
    (p as { baseUrl: string }).baseUrl = 'https://aniwatchtv.to';
    return p;
}

/** `slug?ep=KEY` → `slug$episode$KEY` (Consumet Hianime). */
export function normalizedAnimeEpisodeIdToConsumet(animeEpisodeId: string): string {
    const idx = animeEpisodeId.indexOf('?ep=');
    if (idx < 1) return animeEpisodeId.replace(/\?ep=/i, '$episode$');
 ...
 */
function mapStreamServerToConsumet(server: string, mod: any): string | undefined {
    const s = server.toLowerCase();
    if (s.includes('vid') && s.includes('stream')) return mod.StreamingServers.VidStreaming;
    if (s.includes('tape')) return mod.StreamingServers.StreamTape;
    if (s.includes('sb')) return mod.StreamingServers.StreamSB;
    if (s.includes('hd-2') || (s.includes('vid') && s.includes('cloud'))) return mod.StreamingServers.VidCloud;
    if (s.includes('mega')) return mod.StreamingServers.VidStreaming;
...
}): { sources: unknown[]; subtitles: unknown[]; headers?: Record<string, string> } {
    return {
        sources: data.sources || [],
        subtitles: (data.subtitles || []).filter(
            (t: { lang?: string }) => t.lang !== 'Thumbnails' && t.lang !== 'thumbnails'
        ),
...
export async function fetchConsumetHianimeEpisodeSourcesEnvelope(opts: {
    ...
    category: string;
    timeoutMs?: number;
}): Promise<{ status: 200; data: unknown } | null> {
    ...
    try {
        ...
        try {
            const data = await Promise.race([
                ...
                new Promise<never>((_, rej) => setTimeout(() => rej(new Error('timeout')), Math.min(timeoutMs, 14_000))),
            ]);
