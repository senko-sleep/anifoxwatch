...
export function mapUiServerToHianimeRestQuery(raw: string | undefined): string | undefined {
...
export function buildHianimeRestServerTryList(explicit?: string): string[] {
    const preferred = mapUiServerToHianimeRestQuery(explicit);
    const order = [...HIANIME_REST_SERVER_ORDER];
    if (preferred) {
        const known = order.includes(preferred as (typeof order)[number]);
        if (known) {
