...
let scraper: InstanceType<typeof HiAnime.Scraper> | null = null;

function getScraper(): InstanceType<typeof HiAnime.Scraper> {
...
function extractIdsForCategory(
    data: { sub?: EpisodeServersRow[]; dub?: EpisodeServersRow[] } | null | undefined,
    category: 'sub' | 'dub'
): string[] {
    if (!data) return [];
...
export async function fetchLocalAniwatchEpisodeServerIds(
    ...
    try {
        ...
        const ids = extractIdsForCategory(data as { sub?: EpisodeServersRow[]; dub?: EpisodeServersRow[] }, category);
...
export function restServerQueryToAniwatchServer(server: string): HiAnime.AnimeServers {
    const s = server.trim().toLowerCase();
    const allowed = new Set(['hd-1', 'hd-2', 'megacloud', 'streamsb', 'streamtape', 'vidsrc', 't-cloud']);
    if (allowed.has(s)) return s as HiAnime.AnimeServers;
    if (s.includes('mega')) return 'megacloud';
    if (s.includes('tape')) return 'streamtape';
    if (s.includes('sb')) return 'streamsb';
    if (s.includes('vid') && s.includes('cloud')) return 'hd-2';
    return 'hd-1';
}

export async function fetchLocalEpisodeServersEnvelope(
    animeEpisodeId: string,
    timeoutMs = 14_000
