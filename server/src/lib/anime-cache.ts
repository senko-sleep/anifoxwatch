...
export class AnimeCache {
  ...
  static async getAnime(id: string): Promise<any | null> {
    try {
      ...
      const result = await pool.query(
        'SELECT * FROM anime WHERE id = $1 AND updated_at > NOW() - INTERVAL \'30 days\'',
        [id]
      );
    ...
    try {
      ...
      try {
        ...
        for (const ep of episodes) {
          await client.query(
            `INSERT INTO episodes (id, anime_id, number, title, is_filler, has_sub, has_dub, thumbnail, duration)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
            [ep.id, animeId, ep.number, ep.title, ep.isFiller, ep.hasSub, ep.hasDub, ep.thumbnail, ep.duration]
          );
        }
  ...
  static async getSourcePreference(animeId: string): Promise<string | null> {
    try {
      const pool = getPool();
      const result = await pool.query(
        'SELECT preferred_source FROM source_preferences WHERE anime_id = $1',
        [animeId]
      );
  ...
  static async setSourcePreference(animeId: string, source: string): Promise<void> {
    try {
      const pool = getPool();
      await pool.query(
        `INSERT INTO source_preferences (anime_id, preferred_source, last_used)
