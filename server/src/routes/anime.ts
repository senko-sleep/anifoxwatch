...
import { sourcePrefixOf } from '../registered-sources.js';

const router = Router();

/**
 * Does this entry actually go by the name the slug spells out? Used to tell a real AniList
 * id in a slug from a streaming source's own id that merely ends in digits.
 */
function titleMatchesSlug(media: AnimeBase, spoken: string): boolean {
    ...
    for (const title of [media.title, media.titleEnglish, media.titleRomaji, media.titleJapanese]) {
        if (!title) continue;
        const have = words(title);
        const hits = [...want].filter((w) => have.has(w)).length;
        if (hits / want.size >= 0.6) return true;
    }
    return false;
}
...
router.get('/resolve-slug', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // A slug carrying a source prefix is already an id — that is what older links look
        // like ("aniwaves-spy-x-family-season-3-82391"). Searching for it as a name finds
        // whatever happens to share a word, so it is handed back as-is instead.
