...
// .wrangler/tmp/bundle-m87yRB/strip-cf-connecting-ip-header.js
...
app.get("/api/anime/:id/episodes", async (c) => {
  ...
  const ttl = envNum(c.env.CACHE_TTL_ANIME_DETAIL, 3600);
  const { data: result, cacheHit } = await cache.getOrSet(
    cacheKey,
    async () => {
      ...
      if (numericId && !isNaN(numericId)) {
        try {
          const jikanUrl = `${c.env.JIKAN_API_URL}/anime/${numericId}/episodes`;
...
app.get("/api/anime/:id", async (c) => {
  ...
  const numericId = parseInt(idParam, 10);
  if (!numericId || isNaN(numericId)) {
    return c.json({ error: "Invalid anime ID \u2014 must be a numeric AniList ID" }, 400);
  }
  const cache = buildCache(c.env);
  ...
  const { data: result, cacheHit } = await cache.getOrSet(
    ...
    async () => {
      ...
      `;
      const jikanUrl = `${c.env.JIKAN_API_URL}/anime/${numericId}/full`;
      const { data, source } = await queryAniList(c.env, gql, { id: numericId }, jikanUrl);
      if (source === "anilist") {
        const m = data.Media;
        return {
          ...transformAniList(m),
...
// .wrangler/tmp/bundle-m87yRB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-m87yRB/middleware-loader.entry.ts
