...
// Test the new RealDubSource for working dub content
async function testRealDubSource() {
  console.log('🎬 TESTING REALDUB SOURCE');
  console.log('=====================================\n');
  ...
  // Test anime that should have dub content
  const testAnime = [
  ...
  for (const anime of testAnime) {
    ...
    const searchData = await makeRequest(`/api/anime/search?q=${encodeURIComponent(anime.query)}&source=RealDubSource`);
    ...
    console.log(`   Episodes: ${firstResult.episodes}`);
    
    // Test anime info
    console.log('\n2. Testing anime info...');
    const infoData = await makeRequest(`/api/anime/info?id=${encodeURIComponent(firstResult.id)}&source=RealDubSource`);
    ...
    if (infoData.error) {
      ...
      console.log(`   Description: ${infoData.description?.substring(0, 100)}...`);
    }
    
    // Test episodes
    console.log('\n3. Testing episodes...');
    const epData = await makeRequest(`/api/anime/episodes?id=${encodeURIComponent(firstResult.id)}&source=RealDubSource`);
    ...
    console.log(`   Has sub: ${firstEp.hasSub}`);
    
    // Test dub stream
    console.log('\n4. Testing dub stream...');
    const dubData = await makeRequest(`/api/stream/watch/${encodeURIComponent(firstEp.id)}?category=dub&source=RealDubSource`);
    ...
    if (dubData.error) {
      ...
      if (dubData.category === 'dub' && dubData.audioLanguage === 'en' && !dubData.dubFallback) {
        ...
        // Check if it's using actual content or placeholder
        if (source.url.includes('BigBuckBunny.mp4')) {
          console.log(`⚠️  Using placeholder video (for testing)`);
        } else {
          console.log(`✅ Using actual video stream`);
        }
      } else {
    ...
    // Test sub stream (should return empty for dub-only source)
    console.log('\n5. Testing sub stream (should be empty)...');
    const subData = await makeRequest(`/api/stream/watch/${encodeURIComponent(firstEp.id)}?category=sub&source=RealDubSource`);
  ...
  // Test trending
  console.log('Testing trending dub anime...');
  ...
  if (trendingData.error) {
    ...
    console.log(`⚠️  No trending anime found`);
  } else {
    console.log(`✅ Found ${trendingData.length} trending dub anime`);
    trendingData.slice(0, 5).forEach((anime, i) => {
      console.log(`   ${i + 1}. ${anime.title} (${anime.episodes} eps, dubCount: ${anime.dubCount})`);
    });
  }
  ...
  console.log('Testing cross-source dub fallback with RealDubSource...');
  const fallbackData = await makeRequest('/api/stream/watch/realdub-attack-on-titan-episode-1?category=dub');
  ...
  console.log('\n=====================================');
  console.log('REALDUB SOURCE TEST COMPLETE');
  console.log('=====================================\n');
  ...
  console.log('\n✅ RealDubSource Status:');
  console.log('- 20 known dub anime with confirmed dub availability');
  console.log('- Dub-only source (no sub content)');
  console.log('- Priority source for dub requests');
  console.log('- Uses placeholder content for testing');
  console.log('- Ready for actual dub stream integration');
}
