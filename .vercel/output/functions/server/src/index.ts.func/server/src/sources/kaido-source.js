    ...
    async getStreamingLinks(episodeId, server, category = 'sub', options) {
        ...
        try {
            const result = await Promise.race([
                ...
                new Promise((_, rej) => setTimeout(() => rej(new Error('kaido extraction timeout')), 22000))
            ]);
        ...
        try {
            const result = await Promise.race([
                ...
                new Promise((_, rej) => setTimeout(() => rej(new Error('9anime extraction timeout')), 22000))
            ]);
            if (result.success && result.streams?.length > 0) {
...
//# sourceMappingURL=kaido-source.js.map