...
router.get('/search', async (req, res) => {
    ...
    try {
        ...
        logger.info(`🔍 Search: "${query}" page ${pageNum} source: ${source || 'auto'} mode: ${searchMode}`, { query, page: pageNum, source: source, mode: searchMode }, 'AnimeRoutes');
        const result = await sourceManager.search(query, pageNum, source, { mode: searchMode });
        const duration = timer.end();
        logger.info(`✅ Search completed: "${query}" returned ${result.results?.length || 0} results in ${duration}ms`, { query, page: pageNum, source: source, mode: searchMode, duration, results: result.results?.length }, 'AnimeRoutes');
        ...
        const duration = timer.end();
        const { q } = req.query;
        logger.error(`❌ Search error for "${q}"`, error, { query: q, duration }, 'AnimeRoutes');
...
router.get('/trending', async (req, res) => {
    try {
        ...
        const pageNum = parseInt(page, 10) || 1;
        const result = await sourceManager.getTrending(pageNum, source);
 ...
 */
router.get('/hero-spotlight', async (req, res) => {
    try {
        ...
        const malOn = Boolean(process.env.MAL_CLIENT_ID?.trim() || process.env.MYANIMELIST_CLIENT_ID?.trim());
        res.json({
            results,
        ...
        logger.error('Hero spotlight error', error, {}, 'AnimeRoutes');
...
router.get('/latest', async (req, res) => {
    try {
        const { page = '1', source } = req.query;
        const pageNum = parseInt(page, 10) || 1;
        const result = await sourceManager.getLatest(pageNum, source);
        res.json({ results: result, source: source || 'default' });
    }
    catch (error) {
...
router.get('/top-rated', async (req, res) => {
    try {
        ...
        const result = await sourceManager.getTopRated(pageNum, limitNum, source);
...
router.get('/schedule', async (req, res) => {
    try {
        ...
        const result = await anilistService.getAiringSchedule(start_date, end_date, pageNum, 50);
...
router.get('/seasonal', async (req, res) => {
    try {
        ...
        const result = await anilistService.getSeasonalAnime(yearNum, season, pageNum, 25);
        // Extract results and pageInfo based on the response structure
        const seasonalResult = result;
        res.json({
...
router.get('/genre/:genre', async (req, res) => {
    try {
        ...
        const pageNum = parseInt(page, 10) || 1;
        const result = await sourceManager.getAnimeByGenre(genre, pageNum, source);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.get('/genre-anilist/:genre', async (req, res) => {
    try {
        const { genre } = req.params;
...
router.get('/filter', async (req, res) => {
    try {
        const { type, genre, status, year, season, page = '1', limit = '20', sort = 'rating', order = 'desc', source } = req.query;
        const pageNum = parseInt(page, 10) || 1;
...
router.get('/browse', async (req, res) => {
    ...
    try {
        const { type, genre, genres, status, year, startYear, endYear, page = '1', limit = '25', sort = 'popularity', order = 'desc', source, mode = 'safe' } = req.query;
        const pageNum = parseInt(page, 10) || 1;
        const limitNum = parseInt(limit, 10) || 25;
        const genreParam = genres || genre;
        const parsedGenres = genreParam ? genreParam.split(',').map(g => g.trim()) : [];
        const browseMode = mode;
...
router.get('/random', async (req, res) => {
    try {
        ...
        const result = await sourceManager.getRandomAnime(source);
        if (!result) {
