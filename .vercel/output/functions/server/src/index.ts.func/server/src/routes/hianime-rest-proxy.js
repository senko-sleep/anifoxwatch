...
import { fetchLocalAniwatchEpisodeSourcesJson, fetchLocalEpisodeServersEnvelope, } from '../utils/hianime-local-aniwatch.js';
import { normalizeAnimeEpisodeIdForHianimeRest } from '../utils/hianime-rest-servers.js';
function getHianimeRestBase() {
...
function upstreamJsonLooksSuccessful(fetchResp, text) {
    if (!fetchResp.ok)
        return false;
    try {
...
const router = Router();
router.get('/episode/servers', async (req, res) => {
...
router.get('/episode/sources', async (req, res) => {
    ...
    res.status(200).json({
        ...
        degraded: true,
        message: 'No sources extracted (upstream HiAnime REST, Consumet, and aniwatch npm all failed). Retry from a non-serverless host or check mirror availability.',
    });
});
export default router;
//# sourceMappingURL=hianime-rest-proxy.js.map