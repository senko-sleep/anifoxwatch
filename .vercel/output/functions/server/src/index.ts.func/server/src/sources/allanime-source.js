...
function decryptTobeparsed(tbp, method) {
    try {
        ...
        try {
            ...
            console.error('[AllAnime] Failed to decrypt with correct key:', e.message);
        ...
        console.error('[AllAnime] Failed to decrypt tobeparsed:', e.message);
        throw e;
    }
}
export class AllAnimeSource extends BaseAnimeSource {
    ...
    async healthCheck(options) {
        try {
            const data = await this.gqlQuery('{shows(search:{query:"Naruto"},limit:1,page:1,countryOrigin:ALL){edges{_id}}}', options);
    ...
    async search(query, page = 1, filters, options) {
        try {
            ...
            const data = await this.gqlQuery(`{shows(search:{query:"${escapedQuery}"},limit:20,page:${page},countryOrigin:ALL){edges{_id,name,thumbnail,description,genres,status,availableEpisodesDetail}}}`, options);
            const results = (data?.shows?.edges || []).map((s) => this.mapShow(s));
            return {
                results,
    ...
    async getAnime(id, options) {
        try {
            ...
            const data = await this.gqlQuery(`{show(_id:"${escapedId}"){_id,name,thumbnail,description,genres,status,availableEpisodesDetail}}`, options);
            if (!data?.show)
                return null;
    ...
    async getEpisodes(animeId, options) {
        try {
            ...
            const escapedId = showId.replace(/"/g, '\\"');
            const data = await this.gqlQuery(`{show(_id:"${escapedId}"){availableEpisodesDetail}}`, options);
    ...
    async getStreamingLinks(episodeId, server, category = 'sub', options) {
        try {
            ...
            const escapedId = showId.replace(/"/g, '\\"');
            const data = await this.gqlQuery(`{episode(showId:"${escapedId}",translationType:${translationType},episodeString:"${epNum}"){sourceUrls}}`, options);
            ...
            for (const src of sorted) {
                ...
                if (raw.startsWith('/apivtwo/clock')) {
                    try {
                        ...
                        const links = clockResp.data?.links || [];
    ...
    async getTrending(page = 1, options) {
        try {
            const data = await this.gqlQuery(`{shows(search:{sortBy:"Top",allowAdult:false},limit:20,page:${page},countryOrigin:JP){edges{_id,name,thumbnail,description,genres,status,availableEpisodesDetail}}}`, options);
            return (data?.shows?.edges || []).map((s) => this.mapShow(s));
        }
        catch (error) {
    ...
    async getLatest(page = 1, options) {
        try {
            const data = await this.gqlQuery(`{shows(search:{sortBy:"Update",allowAdult:false},limit:20,page:${page},countryOrigin:JP){edges{_id,name,thumbnail,description,genres,status,availableEpisodesDetail}}}`, options);
            return (data?.shows?.edges || []).map((s) => this.mapShow(s));
        }
        catch (error) {
...
//# sourceMappingURL=allanime-source.js.map