...
export class StreamingVerifier {
    sourceManager;
    testAnimeId = 'naruto'; // Naruto as the test case
     ...
     */
    async testSearch(source) {
        for (let attempt = 0; attempt < this.MAX_RETRIES; attempt++) {
            try {
                const result = await Promise.race([
                    this.sourceManager.search(this.testAnimeId, 1, source),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Search timeout')), this.TIMEOUT_MS))
                ]);
     ...
     */
    async testAnimeInfo(source) {
        const animeId = `${source.toLowerCase()}-${this.testAnimeId}`;
        try {
            return await Promise.race([
                this.sourceManager.getAnime(animeId),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Anime info timeout')), this.TIMEOUT_MS))
            ]);
        }
        catch {
            return null;
     ...
     */
    async testEpisodes(source) {
        const animeId = `${source.toLowerCase()}-${this.testAnimeId}`;
        try {
            return await Promise.race([
                this.sourceManager.getEpisodes(animeId),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Episodes timeout')), this.TIMEOUT_MS))
            ]);
        }
        catch {
            return [];
    ...
    async testStreaming(source) {
        ...
        try {
            ...
            const streamData = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Streaming timeout')), this.TIMEOUT_MS))
            ]);
    ...
    async getStreamQuality(source) {
        ...
        try {
            ...
            const streamData = await this.sourceManager.getStreamingLinks(episodes[0].id, undefined, 'sub');
