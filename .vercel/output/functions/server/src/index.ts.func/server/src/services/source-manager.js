import { AnimePaheDirectSource, AnimeKaiSource, NineAnimeSource, WatchHentaiSource, HanimeSource, AkiHSource, ConsumetSource, AnimeFLVSource, GogoanimeSource, AllAnimeSource, KaidoSource, ZoroSource, MiruroSource, AniwaveSource, AnixSource, DirectDownloadSource, KickassAnimeSource, YugenAnimeSource, AnimeSugeSource, } from '../sources/index.js';
import { logger, PerformanceTimer } from '../utils/logger.js';
import { AnimeCache } from '../lib/anime-cache.js';
import { animeCache, episodesCache } from '../lib/memory-cache.js';
import { anilistService } from './anilist-service.js';
import { reliableRequest, withTimeout } from '../middleware/reliability.js';
...
export class SourceManager {
    sources = new Map();
    primarySource = 'AnimeFLV';
    healthStatus = new Map();
    sourceMetadata = new Map();
    sourceOrder = [...REGISTERED_SOURCE_NAMES];
    // Source capabilities mapping
    sourceCapabilities = new Map([
    ...
    sourceRequestCounts = new Map();
    sourceRateLimits = new Map();
    // Performance tracking for smart source selection
    recentLatencies = new Map();
    LATENCY_SAMPLE_SIZE = 10;
    SUCCESS_RATE_WINDOW = 50; // Number of requests to track
    ...
    cleanupMemory() {
        ...
        const staleEntries = this.requestQueue.filter((entry) => entry.queuedAt && (now - entry.queuedAt) > staleThreshold);
    ...
    async performInitialHealthCheck() {
        ...
        for (const name of prioritySources) {
            ...
            try {
                ...
                const isHealthy = await Promise.race([
                    source.healthCheck({ timeout: 8000 }),
                    new Promise((resolve) => setTimeout(() => resolve(true), 8000) // Default to true on timeout
                    )
                ]);
        ...
        logger.healthSummary(onlineSources.length, this.sources.size, onlineSources.map(s => s.name), offlineSources.map(s => s.name), duration);
    }
    /**
     * Internal helper to execute a source method reliably with timeout, retries, and circuit breaker
     */
    async executeReliably(sourceName, operation, fn, options = {}) {
        return this.enqueueRequest((signal) => reliableRequest(sourceName, operation, (innerSignal) => fn(innerSignal), { ...options, signal }), operation, options);
    }
    /**
     * Enqueue a request for concurrency control
     */
    async enqueueRequest(fn, context, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            entry.resolve = ((val) => { clearTimeout(queueTimer); origResolve(val); });
            entry.reject = ((err) => { clearTimeout(queueTimer); origReject(err); });
            this.processQueue();
        });
    }
    async processQueue() {
        ...
        const safetyTimer = setTimeout(() => {
            ...
            try {
                request.reject(new Error(`Safety timeout for: ${request.context}`));
            }
            catch { /* already resolved */ }
            this.processQueue();
        }, requestTimeout * 2);
        try {
            const result = await withTimeout((signal) => request.fn(signal), requestTimeout, { operation: request.context }, request.options.signal);
    ...
    async checkAllHealth() {
        ...
        const checks = Array.from(this.sources.entries()).map(async ([name, source]) => {
            ...
            try {
                const isHealthy = await reliableRequest(name, 'healthCheck', (signal) => source.healthCheck({ signal }), { timeout: 8000, maxAttempts: 1 });
     ...
     */
    async healthCheck(sourceName, options) {
    ...
    getBestSource(options) {
        ...
        for (const name of this.sourceOrder) {
            ...
            if (excludeAdult && name === 'WatchHentai')
                continue;
            const source = this.sources.get(name);
            ...
            if (preferDub && !capabilities.supportsDub)
                continue;
            if (requireSchedule && !capabilities.hasScheduleData)
                continue;
            // Calculate score (higher is better)
            let score = 100;
            // Quality bonus
            if (capabilities.quality === 'high')
                score += 30;
            else if (capabilities.quality === 'medium')
                score += 15;
     ...
     */
    recordSuccess(sourceName, latency) {
     ...
     */
    getSourceCapabilities(sourceName) {
        return this.sourceCapabilities.get(sourceName);
    }
    getAvailableSource(preferred) {
     ...
     */
    resolveSearchSourceAlias(raw) {
        if (!raw || typeof raw !== 'string')
            return undefined;
        const t = raw.trim();
        if (this.sources.has(t))
            return t;
     ...
     */
    isAnimeKaiConsumetEpisodeId(id) {
        if (id.includes('?ep='))
            return false;
        return /\$ep=\d+/.test(id);
    }
     ...
     */
    hasKnownSourcePrefix(id) {
        const lowerId = id.toLowerCase();
        if (this.knownPrefixes.some(prefix => lowerId.startsWith(prefix)))
            return true;
     ...
     */
    episodeIdToFallbackSearchTitle(episodeId) {
        let slug = episodeId.split('?')[0];
        const dollar = slug.indexOf('$');
        if (dollar !== -1)
            slug = slug.slice(0, dollar);
     ...
     */
    buildSourceId(rawId, sourceName) {
        const prefixMap = {
            'Kaido': 'kaido-',
            'AnimePahe': 'animepahe-',
            'AnimeKai': 'animekai-',
     ...
     */
    resolveStreamingEpisodeId(episodeId, source, primarySource, hasSourcePrefix, rawId) {
        ...
        if (source === primarySource || !hasSourcePrefix)
            return episodeId;
        return this.buildSourceId(rawId, source.name);
    }
    getStreamingSource(id) {
        const lowerId = id.toLowerCase();
    ...
    // ============ ANIME DATA METHODS ============
    async search(query, page = 1, sourceName, options) {
        ...
        if (mode === 'adult') {
            ...
            try {
                const searchPromises = adultSources.map(source => source.search(query, page)
                ...
                results.forEach(r => {
                    ...
                    if (r.totalPages > maxTotalPages)
                        maxTotalPages = r.totalPages;
                    if (r.hasNextPage)
                        hasNextPage = true;
                });
        ...
        if (resolvedSource) {
            ...
            try {
                ...
                console.log(`❌ [SourceManager] Search failed with source "${resolvedSource}": ${error.message}`);
                logger.error(`Search failed with source ${resolvedSource}`, error, { query });
                throw error;
            }
        }
        ...
        try {
            ...
            const searchPromises = sourcesToTry.map(source => this.executeReliably(source.name, 'search', (signal) => source.search(query, page, undefined, { signal }))
                .then(res => {
                ...
                console.log(`❌ [SourceManager] Search failed with source "${source.name}": ${error.message}`);
            ...
            if (sortedResults.length === 0) {
                ...
                try {
                    ...
                    console.warn(`[SourceManager] AniList search fallback failed:`, e.message);
                }
            }
            else {
            ...
            logger.error(`Multi-source search failed`, error, { query });
    ...
    anilistEnrichmentCache = new Map();
    enrichmentQueue = new Map();
    /**
     ...
     */
    async enrichWithAniListData(results) {
    ...
    SOURCE_PRIORITY = ['animekai-', 'animepahe-', 'allAnime-', 'consumet-', 'animeflv-'];
    sourceRank(id) {
    ...
    async getAnime(id) {
        ...
        if (lowerId.startsWith('anilist-')) {
            ...
            try {
                ...
                logger.info(`[SourceManager] Looking for streaming match for AniList titles: ${uniqueTitles
                    .map((t) => `"${t}"`)
                    .join(' | ')} (type: ${anilistData.type})`);
                let streamingMatch = null;
                ...
                logger.error(`[SourceManager] getAnime failed for AniList ID ${anilistId}:`, error);
                return null;
            }
        }
        ...
        if (source) {
            try {
                ...
                if (result && result.title && result.title !== 'Unknown') {
                    ...
                    if (process.env.POSTGRES_URL) {
                        AnimeCache.setAnime(result).catch((err) => logger.warn(`[SourceManager] Failed to cache anime:`, { error: err.message }));
                    }
                    return result;
                }
            }
            catch (error) {
                console.log(`[SourceManager] getAnime failed for ${source.name}:`, error.message);
            }
        }
        ...
        if (!hasPrefix) {
            ...
            if (titleFromSlug.length >= 3) {
                ...
                try {
                    ...
                    console.log(`[SourceManager] getAnime: Title fallback failed:`, err.message);
                }
            }
        }
        return null;
    ...
    async getEpisodes(animeId) {
        ...
        if (animeId.toLowerCase().startsWith('anilist-')) {
            ...
            if (!isNaN(numericId)) {
                try {
                    ...
                    console.log(`   ❌ AniList resolution failed: ${err.message}`);
                }
            }
            timer.end();
            return [];
        ...
        if (!hasSourcePrefix) {
            ...
            if (primarySource?.isAvailable) {
                ...
                try {
                    const episodes = await this.executeReliably(primarySource.name, 'getEpisodes', (signal) => primarySource.getEpisodes(animeId, { signal }), { timeout: 15000 });
                    if (episodes && episodes.length > 0) {
                        const duration = Date.now() - startTime;
                        ...
                        if (process.env.POSTGRES_URL) {
                            AnimeCache.setEpisodes(animeId, episodes).catch((err) => logger.warn(`[SourceManager] Failed to cache episodes:`, { error: err.message }));
                        }
                        timer.end();
                        return episodes;
                    }
                    ...
                    console.log(`   ❌ Primary source failed: ${err.message}`);
                }
            }
            ...
            if (titleFromSlug.length >= 3) {
                ...
                try {
                    ...
                    console.log(`   ❌ Title search failed: ${err.message}`);
                }
            }
            ...
            if (kaidoSource?.isAvailable && primarySource?.name !== 'Kaido') {
                ...
                try {
                    const episodes = await this.executeReliably('Kaido', 'getEpisodes', (signal) => kaidoSource.getEpisodes(animeId, { signal }), { timeout: 12000 });
                    if (episodes && episodes.length > 0) {
                        const duration = Date.now() - startTime;
                    ...
                    console.log(`   ❌ Kaido failed: ${err.message}`);
                }
            }
            timer.end();
            return [];
        ...
        let episodes = [];
        try {
            episodes = await this.executeReliably(primarySource.name, 'getEpisodes', (signal) => primarySource.getEpisodes(animeId, { signal }), { timeout: 15000 });
        }
        catch (err) {
            console.log(`   ❌ Primary source failed: ${err.message}`);
        }
        ...
        if (needsEnrichment) {
            ...
            try {
                ...
                if (kaidoSource?.isAvailable && searchQuery.length > 3) {
                    ...
                    if (searchResult && searchResult.results?.length > 0) {
                        ...
                        for (const result of searchResult.results) {
                            ...
                            if (rNorm === qNorm)
                                score += 200;
                            if (querySeason === 0 && /\b(season|final|2nd|3rd|\d+th)\b/i.test(rLow))
                                score -= 50;
                            if (score > bestScore) {
                                bestScore = score;
                                bestMatch = result;
                        ...
                        if (bestMatch) {
                            ...
                            const kaidoEpisodes = await Promise.race([
                                this.executeReliably('Kaido', 'getEpisodes', (signal) => kaidoSource.getEpisodes(bestMatch.id, { signal }), { timeout: 10000 }),
                                new Promise((resolve) => setTimeout(() => resolve([]), 10000))
                            ]);
                            // Use anime-level dubCount from search results when episode-level hasDub is unreliable
                            const animeDubCount = bestMatch.dubCount ?? 0;
                            const animeSubCount = bestMatch.subCount ?? 0;
                ...
                console.log(`   ℹ️ Enrichment failed (non-fatal): ${err.message}`);
            }
        }
        ...
        const rawId = this.extractRawId(animeId);
        const backupSourceNames = REGISTERED_SOURCE_NAMES.filter((name) => name !== primarySource.name && name !== 'WatchHentai' && name !== 'Hanime');
        ...
        for (const name of backupSourceNames) {
            ...
            try {
                ...
                const searchResult = await this.executeReliably(name, 'search', (signal) => source.search(strippedTitle, 1, { signal }), { timeout: 8000 });
                if (searchResult && searchResult.results?.length > 0) {
                    ...
                    const backupEpisodes = await this.executeReliably(name, 'getEpisodes', (signal) => source.getEpisodes(bestMatchId, { signal }), { timeout: 12000 });
                ...
                console.log(`   ❌ Backup ${name} failed: ${err.message}`);
            }
        }
        timer.end();
    ...
    async getTrending(page = 1, sourceName) {
        ...
        if (sourceName) {
            ...
            try {
                ...
                logger.error(`getTrending failed on ${source.name}`, error, { page }, 'SourceManager');
                return [];
            }
        }
        ...
        const sourcePromises = availableSources.map(source => Promise.race([
            source.getTrending(page).then(res => ({ source: source.name, results: res || [] })),
            new Promise(r => setTimeout(() => r({ source: source.name, results: [] }), HOMEPAGE_TIMEOUT))
        ]).catch(() => ({ source: source.name, results: [] })));
        ...
        const anilistPromise = Promise.race([
            anilistService.advancedSearch({ sort: ['TRENDING_DESC'], perPage: 24, page })
                .then(res => ({ source: 'AniList', results: res.results || [] })),
            new Promise(r => setTimeout(() => r({ source: 'AniList', results: [] }), HOMEPAGE_TIMEOUT))
        ]).catch(() => ({ source: 'AniList', results: [] }));
    ...
    async getLatest(page = 1, sourceName) {
        ...
        if (sourceName) {
            ...
            try {
                ...
                logger.error(`getLatest failed on ${source.name}`, error, { page }, 'SourceManager');
                return [];
            }
        }
        // FAST PATH: bypass executeReliably queue entirely for homepage data.
    ...
    async getTopRated(page = 1, limit = 10, sourceName) {
        const source = this.getAvailableSource(sourceName);
        if (!source) {
        ...
        try {
            ...
            console.log(`❌ [SourceManager] getTopRated failed on ${source.name}: ${error.message}`);
        ...
        try {
            ...
            console.warn(`[SourceManager] AniList top-rated fallback failed:`, e.message);
        }
        return [];
    }
    /**
    ...
    async getFilteredAnime(filters) {
        ...
        try {
            ...
            if (filters.genres && filters.genres.length > 0) {
                filtered = filtered.filter(a => {
                    if (!a.genres || a.genres.length === 0)
                        return false;
                    return filters.genres.some(g => a.genres.some(ag => ag.toLowerCase().includes(g.toLowerCase())));
                });
            }
            // Filter by status
            if (filters.status) {
            ...
            logger.error(`Filtered anime failed for ${source.name}`, error, filters, 'SourceManager');
    ...
    async browseAnime(filters) {
        ...
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Browse timeout after 15s`)), 15000))
            ]);
            ...
            logger.error(`Browse failed with source ${source.name}: ${errorMessage}`, error, filters);
            // Failover
            const fallback = this.getAvailableSource();
    ...
    async _executeBrowse(source, filters) {
        const page = filters.page || 1;
        ...
        if (!isPaginatedResult) {
            ...
            if (filters.genres && filters.genres.length > 0 && typeof source.getByGenre === 'function') {
                try {
                    const genre = filters.genres[0];
                    const genreResult = await source.getByGenre(genre, page);
                    ...
                    if (genreResult.results && genreResult.results.length > 0) {
                        ...
                        const latestResult = await source.getLatest(page);
            ...
            if (!isPaginatedResult && typeof source.getByType === 'function' && filters.type && !isAdultSource) {
                try {
                    const typeResult = await source.getByType(filters.type, page);
            ...
            if (!isPaginatedResult && isAdultSource && !filters.type && typeof source.getLatest === 'function') {
                try {
                    const latestResult = await source.getLatest(page);
                    if (latestResult.length > 0) {
                        finalResults = latestResult;
            ...
            if (!isPaginatedResult) {
                ...
                const isAdultSource = ['WatchHentai', 'Hanime'].includes(source.name);
                const backupSourceNames = isAdultSource ? [] : REGISTERED_SOURCE_NAMES.filter((n) => n !== source.name && n !== 'WatchHentai' && n !== 'Hanime');
                ...
                const results = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Multi-source fetch timeout')), 12000))
                ]).catch(() => [[]]);
                // Combine all results
    ...
    async getEpisodeServers(episodeId) {
        ...
        if (!hasSourcePrefix) {
            const slugSources = ['AnimeKai', '9Anime'];
            const toTry = slugSources
                .map(n => this.sources.get(n))
                .filter((s) => !!s?.isAvailable && !!s.getEpisodeServers);
            for (const source of toTry) {
                try {
                    console.log(`   ⏳ Trying ${source.name} with slug: ${episodeId}`);
                    const servers = await this.executeReliably(source.name, 'getEpisodeServers', (signal) => source.getEpisodeServers(episodeId, { signal }), { timeout: 8000 });
        ...
        for (const source of sourcesToTry) {
            ...
            try {
                const servers = await this.executeReliably(source.name, 'getEpisodeServers', (signal) => source.getEpisodeServers(idToUse, { signal }), { timeout: 8000 });
                if (servers && servers.length > 0) {
                    const duration = Date.now() - startTime;
                    console.log(`   ✅ Got ${servers.length} servers from ${source.name} in ${duration}ms`);
                    timer.end();
     ...
     */
    async getStreamingLinks(episodeId, server, category = 'sub', episodeNum, anilistId) {
        ...
        const backupNames = REGISTERED_SOURCE_NAMES.filter((n) => n !== primarySource?.name);
        for (const name of backupNames) {
            if (isAdultContent && name !== 'WatchHentai' && name !== 'Hanime')
                continue;
            if (!isAdultContent && (name === 'WatchHentai' || name === 'Hanime'))
                continue;
            const source = this.sources.get(name);
        ...
        const buildStreamingPickOrder = (epId) => {
            ...
            if (!watchShape)
                return [...STREAM_PRIORITY, 'cross-source'];
            const preferred = [
                'Miruro',
                'Consumet',
                'AllAnime',
        ...
        const result = await new Promise((resolveStream) => {
            let pending = 0;
            const pickBestAndResolve = (force = false) => {
                ...
                const hasRealStream = (r) => r.data.sources.some((s) => {
                    const u = s.originalUrl || s.url || '';
                ...
                const hasProxyableStream = (r) => r.data.sources.some((s) => {
                    if (s.ipLocked)
                        return false;
                    const u = s.originalUrl || s.url || '';
                    // Streamtape direct videos are IP-locked even without the flag
                    if ((u.includes('streamtape') || u.includes('tapecontent')) && u.includes('get_video'))
                        return false;
                    return u.includes('.m3u8') || u.includes('.mp4') || u.includes('.mpd');
                });
                ...
                const duration = Date.now() - startTime;
                logger.streamingSuccess('unknown', episodeId, best.source, best.data.sources[0]?.quality || 'unknown', duration);
                timer.end();
            ...
            for (const source of finalSources) {
                ...
                const streamReliabilityOpts = usesPuppeteerStream
                    ...
                    : usesMiruroStack
                        ...
                        : { timeout: 25_000, maxAttempts: 1 };
                this.executeReliably(source.name, 'getStreamingLinks', (signal) => source.getStreamingLinks(idToUse, server, category, { signal, episodeNum, anilistId }), streamReliabilityOpts)
                    .then(data => {
                    ...
                    .catch(err => {
                    console.log(`   ❌ ${source.name} failed: ${err.message?.substring(0, 80)}`);
                    allResults.push({ source: source.name, data: { sources: [], subtitles: [] }, success: false });
                })
                    .finally(onDone);
            }
            ...
            Promise.race([
                ...
                new Promise((resolve) => setTimeout(() => resolve(null), CROSS_SOURCE_FALLBACK_MAX_MS)),
            ])
                .then(data => {
                ...
                .catch(err => {
                console.log(`   ❌ Cross-source fallback failed: ${err.message?.substring(0, 80)}`);
                allResults.push({ source: 'cross-source', data: { sources: [], subtitles: [] }, success: false });
            })
                .finally(onDone);
        });
        return result;
     ...
     */
    async tryAnimeKaiCompoundFromWatchQueryEpisode(watchEpisodeId, catalogEpisode, server, category = 'sub', extra) {
        const compound = reconstructAnimeKaiCompoundFromWatchUrl(watchEpisodeId, catalogEpisode);
        if (!compound)
            return null;
        const kai = this.sources.get('AnimeKai');
        if (!kai?.getStreamingLinks)
            return null;
        const id = `animekai-${compound}`;
        try {
            const data = await this.executeReliably('AnimeKai', 'getStreamingLinks', (signal) => kai.getStreamingLinks(id, server, category, {
                signal,
     ...
     */
    async crossSourceStreamingFallback(episodeId, server, category = 'sub', hintEpisodeNum, anilistId) {
        ...
        if (anilistId) {
            try {
                ...
                const res = await axios.post('https://graphql.anilist.co', { query, variables: { id: anilistId } }, { timeout: 5000, headers: { 'Content-Type': 'application/json' } });
        ...
        if (dollar !== -1)
            animeSlug = animeSlug.slice(0, dollar);
        const kaidoSource = this.sources.get('Kaido');
        ...
        const consumetSources = ['AllAnime', 'Gogoanime', 'AnimeKai', 'AnimePahe', 'Consumet']
            .map(n => ({ name: n, src: this.sources.get(n) }))
            .filter(({ src }) => src?.isAvailable && src.getStreamingLinks);
        const crossResults = await Promise.allSettled(consumetSources.map(async ({ name: srcName, src }) => {
            ...
            if (!targetEp)
                throw new Error(`ep ${targetEpNum} not in ${episodes.length} episodes`);
    ...
    async getStreamingLinksFromSource(sourceName, episodeId, server, category = 'sub') {
        const src = this.sources.get(sourceName);
        if (!src?.isAvailable || !src.getStreamingLinks)
            return { sources: [], subtitles: [] };
     ...
     */
    async tryAllAnimeFallback(episodeId, category = 'sub', episodeNum, anilistId) {
        const allAnime = this.sources.get('AllAnime');
        if (!allAnime?.isAvailable || !allAnime.getStreamingLinks)
            return null;
        ...
        try {
            ...
            console.log(`[AllAnime fallback] Failed: ${e.message}`);
            return null;
        }
    }
    async tryAllAnimeFallbackWithSubFallback(episodeId, category = 'sub', episodeNum, anilistId) {
        ...
        try {
            ...
            for (const sourceName of this.sourceOrder) {
                ...
                try {
                    ...
                    failedSources.push(sourceName);
                    sourceErrors.push({ source: sourceName, error: error.message });
                    logger.error(`Search failed on ${sourceName}: ${error.message} `, error, undefined, 'SourceManager');
                }
            }
            if (results.length === 0) {
            ...
            logger.error(`❌ SEARCH CRITICAL ERROR: ${error.message} `, error, undefined, 'SourceManager');
            logger.error(`   Query: "${query}"`, new Error('Query info'), undefined, 'SourceManager');
            logger.error(`   Page: ${page} `, new Error('Page info'), undefined, 'SourceManager');
            return { results: [], totalPages: 0, currentPage: page, hasNextPage: false, source: 'none' };
        }
     ...
     */
    async getAnimeByGenreAniList(genre, page = 1) {
        try {
            ...
            const titlesNeedingSearch = [];
            const titleToAnimeMap = new Map();
            ...
            logger.error(`AniList genre search failed for "${genre}"`, error, undefined, 'SourceManager');
     ...
     */
    streamingLookupTable = new Map();
    streamingLookupBuilt = false;
    /**
     * Build streaming lookup table on first use (lazy initialization)
     */
    async buildStreamingLookupTable() {
        if (this.streamingLookupBuilt)
            return;
     ...
     */
    findStreamingMatchInstant(title) {
        if (!title || !this.streamingLookupBuilt)
            return null;
     ...
     */
    async findStreamingMatchSearch(title) {
        try {
            const source = this.getAvailableSource();
            if (!source)
                return null;
    ...
    async getAllTrendingAnime() {
        ...
        try {
            ...
            logger.error('Failed to build streaming lookup table', error, undefined, 'SourceManager');
            return [];
        }
    }
    /**
    ...
    searchCache = new Map();
    SEARCH_CACHE_TTL = 10 * 60 * 1000; // 10 minute cache
    // Long-lived cache: AniList numeric ID → resolved streaming ID. Avoids repeated title searches.
    anilistStreamingIdCache = new Map();
    ANILIST_STREAMING_ID_TTL = 30 * 60 * 1000; // 30 minutes
    /**
     * Resolve an AniList id to a playable streaming id.
     * Uses multiple title variants + multi-source search as fallback.
     */
    async resolveAniListToStreamingId(anilistNumericId) {
        if (!Number.isFinite(anilistNumericId) || anilistNumericId <= 0)
            return null;
        ...
        const normalize = (s) => s
            ...
            .replace(/\s+/g, ' ')
            .trim();
        const scoreCandidate = (queryTitle, candidateTitle) => {
        ...
        for (const t of uniqueTitles) {
            ...
            const best = candidates
                .map((c) => ({
                c,
                s: scoreCandidate(anilistData.title, c.title) +
     ...
     */
    findBestMatch(title, results, animeType) {
        if (!results || results.length === 0)
            return null;
     ...
     */
    async findStreamingAnimeByTitle(title, animeType) {
        try {
    ...
    setPreferredSource(sourceName) {
        if (this.sources.has(sourceName)) {
            ...
            this.sourceOrder = [sourceName, ...this.sourceOrder.filter((s) => s !== sourceName)];
            return true;
        }
        return false;
    }
     ...
     */
    async getAnimeByGenre(genre, page = 1, sourceName) {
        ...
        try {
            ...
            const sourceSupportsGenre = (s) => {
                return 'getByGenre' in s;
            };
            if (sourceSupportsGenre(source)) {
                const result = await this.executeReliably(source.name, 'getByGenre', (signal) => source.getByGenre(genre, page, { signal }));
            ...
            logger.error(`Genre search failed for ${source.name}`, error, { genre, page }, 'SourceManager');
            // Try fallback
            const fallback = this.getAvailableSource();
            if (fallback && fallback !== source) {
     ...
     */
    async getRandomAnime(sourceName) {
        ...
        try {
            ...
            logger.error(`Random anime failed for ${source.name}`, error, undefined, 'SourceManager');
            const fallback = this.getAvailableSource();
            if (fallback && fallback !== source) {
