import { buildHianimeRestServersToTry, fetchHianimeRestEpisodeServerIds, HIANIME_REST_FETCH_HEADERS, } from '../utils/hianime-rest-episode-discovery.js';
import { fetchConsumetHianimeEpisodeSourcesEnvelope } from '../utils/hianime-consumet-local.js';
import { fetchLocalAniwatchEpisodeSourcesJson, fetchLocalAniwatchEpisodeServerIds, } from '../utils/hianime-local-aniwatch.js';
import { normalizeAnimeEpisodeIdForHianimeRest } from '../utils/hianime-rest-servers.js';
function mapPayloadToStreaming(data) {
    if (!data?.sources?.length)
        return null;
    const hdr = data.headers;
    const referer = (typeof hdr?.Referer === 'string' && hdr.Referer) ||
        (typeof hdr?.referer === 'string' && hdr.referer) ||
        'https://hianime.to/';
    const sources = data.sources.map((s) => ({
        url: s.url,
        quality: (s.quality || s.type || 'default'),
        isM3U8: Boolean(s.isM3U8),
        originalUrl: s.url,
    }));
    const subtitles = (data.subtitles || []).map((t) => ({
        url: t.url,
        lang: t.lang,
    }));
    return {
...
async function tryRemoteEpisodeSources(base, restEpisodeId, server, category, perAttemptTimeoutMs) {
...
export async function tryFetchHianimeRestStreamingData(opts) {
    ...
    let discovered = null;
    {
        ...
        if (elapsed1 < budget && !discovered?.length) {
            discovered = await fetchLocalAniwatchEpisodeServerIds(restEpisodeId, category, Math.min(discoveryBudget, Math.max(800, budget - elapsed1)));
        }
    }
    ...
    const epSeg = restEpisodeId.includes('?ep=') ? restEpisodeId.split('?ep=')[1]?.split('&')[0]?.split('#')[0] ?? '' : '';
    if (catalogEpisodeFallback != null &&
...
//# sourceMappingURL=hianime-rest-fallback.js.map