...
export class AnimeFoxSource extends BaseAnimeSource {
    name = 'AnimeFox';
    ...
    async search(query, page = 1, _filters, options) {
        try {
            ...
            const results = [];
            $('.film_list-wrap .flw-item, .anime-list .item').each((i, el) => {
                const title = $(el).find('.film-name a, .name a').text().trim();
                const href = $(el).find('.film-name a, .name a').attr('href') || '';
    ...
    async getAnime(id, options) {
        try {
            const animeId = id.replace('animefox-', '');
            ...
            const description = $('.description, .anime-description').text().trim();
            const genres = [];
    ...
    async getEpisodes(animeId, options) {
        try {
            const id = animeId.replace('animefox-', '');
            ...
            const $ = cheerio.load(response.data);
            const episodes = [];
            $('.episodes-list a, .ep-list a').each((i, el) => {
...
//# sourceMappingURL=animefox-source.js.map