...
function getCircuitBreaker(sourceName) {
...
function recordFailure(sourceName) {
    ...
    if (circuit.failureCount >= circuit.maxFailures) {
        circuit.state = 'open';
        logger.circuitBreakerTripped(sourceName, circuit.failureCount, circuit.resetTime);
    }
}
...
function isCircuitBreakerOpen(sourceName) {
    const circuit = getCircuitBreaker(sourceName);
 ...
 */
export async function retry(fn, maxAttempts = 3, delay = 1000, context, parentSignal) {
    let lastError;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            logger.retryAttempt(context?.operation || 'unknown', attempt, maxAttempts, delay);
 ...
 */
export async function withTimeout(fn, timeoutMs = DEFAULT_CIRCUIT_SETTINGS.timeout, context, parentSignal) {
 ...
 */
export async function withCircuitBreaker(sourceName, fn, context) {
 ...
 */
export async function reliableRequest(sourceName, operation, fn, options = {}) {
    const { maxAttempts = 2, // 1 retry for transient failures - prevents single blips from cascading
    timeout = 25000, // 25s timeout — sources need time, especially on Vercel cold starts
    retryDelay = 1000, context: extraContext = {}, signal: parentSignal } = options;
    ...
    try {
        return await retry((signal) => withCircuitBreaker(sourceName, () => withTimeout((timeoutSignal) => fn(timeoutSignal), timeout, requestContext, signal), requestContext), maxAttempts, retryDelay, requestContext, parentSignal);
    }
    catch (error) {
        logger.error(`${operation} failed for ${sourceName}`, error, requestContext);
        throw error;
    }
}
/**
 ...
 */
export function reliabilityMiddleware(req, res, next) {
    ...
    res.on('finish', () => {
        ...
        if (res.statusCode >= 400) {
            logger.error(`API Error ${res.statusCode}`, undefined, {
                ...context,
                statusCode: res.statusCode,
                duration
            });
    ...
    req.reliableRequest = reliableRequest;
    req.retry = retry;
    next();
}
/**
