 ...
 */
async function extractStreamwishUrl(embedUrl) {
    try {
        const resp = await axios.get(embedUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
...
export class AnimeFLVSource extends BaseAnimeSource {
    ...
    async getEpisodes(animeId, options) {
        try {
            ...
            if (episodesMatch) {
                try {
                    const epList = JSON.parse(episodesMatch[1]);
                    epList.forEach((ep) => {
                        const epNum = ep[0];
                        episodes.push({
     ...
     */
    buildEpSlugs(rawSlug, episodeNum) {
        const candidates = [rawSlug];
    ...
    async fetchAniListSlugCandidates(anilistId, episodeNum) {
        try {
            ...
            for (const title of [t.romaji, t.english]) {
                ...
                if (episodeNum && episodeNum > 0)
                    candidates.push(`${slug}-${episodeNum}`);
    ...
    async getStreamingLinks(episodeId, server, category = 'sub', options) {
        try {
            ...
            for (const slug of slugsToTry) {
                try {
                    const r = await axios.get(`${this.baseUrl}/ver/${slug}`, {
                        signal: options?.signal,
                        timeout: options?.timeout || 10000,
                        headers: this.getHeaders(),
            ...
            if (sources.length === 0 && rawEmbeds.length > 0) {
                const pick = rawEmbeds.find(e => e.serverName.includes('sw') || e.serverName.includes('streamwish')) ||
    ...
    mapType(type) {
        ...
        if (t.includes('película') || t.includes('movie'))
            return 'Movie';
        if (t.includes('ova'))
            return 'OVA';
        if (t.includes('ona'))
            return 'ONA';
        if (t.includes('especial') || t.includes('special'))
            return 'Special';
        return 'TV';
    }
}
//# sourceMappingURL=animeflv-source.js.map